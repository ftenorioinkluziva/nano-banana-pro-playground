...
export function PromptEnhancementSuggestions({
  ...
  const SUGGESTION_LABELS = {
    ...
    composition: t.labelComposition,
    ambiance: t.labelAmbiance,
  }

  const SUGGESTION_DESCRIPTIONS = {
    subject: t.descSubject,
    action: t.descAction,
    style: t.descStyle,
    camera: t.descCamera,
    composition: t.descComposition,
    ambiance: t.descAmbiance,
  }
    ...
    <div className="w-full max-w-3xl mx-auto space-y-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-semibold text-white">{t.enhancedPrompt}</h3>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-lg p-4 space-y-2">
        <p className="text-xs text-zinc-500 uppercase tracking-wide">{t.originalPrompt}</p>
      ...
      <div className="bg-blue-900/20 border border-blue-700 rounded-lg p-4 space-y-3">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <p className="text-xs text-blue-500 uppercase tracking-wide mb-2">{t.enhancedPrompt}</p>
      ...
      {availableSuggestions.length > 0 && (
        <div className="space-y-3">
          <p className="text-xs text-zinc-500 uppercase tracking-wide">{t.improvementSuggestions}</p>
      ...
      <div className="flex justify-between items-center text-xs text-zinc-500 px-1">
        <span>{t.originalCharCount.replace("{count}", originalPrompt.length.toString())}</span>
        <span>{t.enhancedCharCount.replace("{count}", enhancedPrompt.length.toString())}</span>
      </div>
      ...
      <div className="flex gap-3 pt-2">
        ...
        >
          {isLoading ? t.processing : t.applyEnhancedPrompt}
        </Button>
        <Button
          onClick={onClose}
          variant="outline"
